  ...
  useEffect(() => {
    return engineClient.on('terminal_output', (data) => {
      setLines((prev) => {
        const next = [
          ...prev,
          { text: data.output || data.text || '', stream: data.stream || 'stdout', ts: Date.now() },
        ];
  ...
  return (
    <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-white/[0.06]">
        ...
        <span className="text-xs font-medium text-white">{t('terminalOutput')}</span>
        {lines.length > 0 && (
      ...
      >
        {lines.length === 0 ? (
          <span className="text-white/20">{t('terminalWaiting')}</span>
        ) : (
          lines.map((line, i) => (
